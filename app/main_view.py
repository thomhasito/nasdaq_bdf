    ...
    fig = pltg.Figure(
        ...
        layout={
            ...
            "xaxis": {
                ...
                "showgrid": True,
                "tickformatstops": [
                    {"dtickrange": [None, "M1"], "value": "%d %b %Y"}, # Show Day-Month-Year for zoomed-in view
                    {"dtickrange": ["M1", None], "value": "%b %Y"},    # Show Month-Year for mid-range view
                ],
                "showline": True,
                "rangeslider": {"visible": True}, # Enable range slider for zooming in/out
            },
            "yaxis": {
...
def display_tabs(stock_pd: pd.DataFrame) -> None:
    ...
    Args:
        stock_pd (pd.DataFrame): Stock data.
    """
    tab_stats, tab_info, tab_history = st.tabs(["📊 Stats", "ℹ️ Info", "📜 History"])
    ...
    with tab_info:
        ...
        date_range_days = (stock_pd[ColumnNames.DATE.value].max() - stock_pd[ColumnNames.DATE.value].min()).days + 1
        all_days_between = pd.date_range(start=stock_pd[ColumnNames.DATE.value].min(), end=stock_pd[ColumnNames.DATE.value].max())
        missing_days = 100 * (1 - count / date_range_days)
        
        not_valid_days = 0 # holidays and weekends
        CHRISTMAS = "12-25"
        NEW_YEAR = "01-01"
        holiday_days = all_days_between.strftime("%m-%d").isin([CHRISTMAS, NEW_YEAR]) # Christmas and New Year
        not_valid_days += holiday_days.sum()
        
        weekend_days = all_days_between.weekday.isin([5, 6]) # Saturday and Sunday
        not_valid_days += weekend_days.sum()
        
        holidays_on_weekends = (holiday_days & weekend_days).sum()
        not_valid_days -= holidays_on_weekends # Remove holidays on weekends (counted twice)
        true_missing_days = 100 * (1 - count / (date_range_days - not_valid_days))
    ...
    with tab_history:
        ...
        options = [i if i <= count else count for i in default_choices if i < count or i == default_choices[-1]]
        ...
        history_df = stock_pd.tail(rows_to_display).sort_values(by=ColumnNames.DATE.value, ascending=False)
        # Format the date and numeric columns
        history_df[ColumnNames.DATE.value] = history_df[ColumnNames.DATE.value].dt.strftime("%Y-%m-%d")
...
def display_stock_data(fig: pltg.Figure, stock_pd: pd.DataFrame, ticker: str, company_name: str) -> None:
    ...
    with st.expander(f"{ticker} - {company_name} {emote_state}", expanded=True):
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"plot_candlestick_{ticker}")
...
for t_idx, ticker in enumerate(selected_tickers):
    ...
    stock_values = ticker_values.filter(ticker_values[ColumnNames.TICKER.value] == ticker)
