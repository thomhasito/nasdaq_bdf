...
# TODO: changer la période pour la détection

st.header("🔍💲 Recherche de stocks profitables")

company_info = get_company_info()
sectors = company_info.select("Sector").distinct().rdd.flatMap(lambda x: x).collect()
...
stocks_df = get_stocks_df("5d", company_tickers)
df_ops = DataFrameOperations(get_logger(), stocks_df)

return_df = df_ops.calculate_daily_return()
highest_yield = df_ops.stocks_with_highest_daily_return(return_df, n_coms)
...
for idx, row in enumerate(highest_yield.distinct().collect()):
    ...
    with col:
        with st.container(border=True):
            ...
            roi = round(row["daily_return"], 2)
            st.text(f"Rendement quotidien: {roi}%")
            if st.button(
                "Ajouter aux tickers suivis",
                key="btn_" + row["Ticker"] + "_" + row["Company"] + "_" + str(idx),
            ):
