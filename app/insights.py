...
def fig_volume_by_day(stock_pd: pd.DataFrame, ticker: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            showline=True,
            tickformatstops=[
                {"dtickrange": [None, "M1"], "value": "%d %b %Y"},  # Daily format
                {"dtickrange": ["M1", None], "value": "%b %Y"}       # Monthly format
            ],
            rangeslider=dict(visible=True)  # Enable range slider for date navigation
        ),
        yaxis=dict(
            title="Volume",
            autorange=True,
...
def fig_sharpe(return_rate_pd: pd.DataFrame, time_window: EnumPeriod, num_avg_days: int = 3, annual_risk_free_rate: float = 0.03) -> go.Figure:
    ...
    return_rate_pd[ColumnNames.SHARPE_RATIO.value] = (return_rate_pd[col_return_name].rolling(num_avg_days).mean() - annual_risk_free_rate) / return_rate_pd[col_return_name].rolling(num_avg_days).std()
    date_col = f"{time_window.value}_period" if time_window != EnumPeriod.DAY else ColumnNames.DATE.value
...
def fig_rsi(rsi_pd: pd.DataFrame, column_rsi_name: str) -> go.Figure:
    ...
    fig.add_hline(
        ...
        line_dash="dash", 
        line_color="red", 
        annotation_text="Overbought Threshold (70)", 
        annotation_position="top right"
    )
    ...
    fig.add_hline(
        ...
        line_dash="dash", 
        line_color="green", 
        annotation_text="Oversold Threshold (30)", 
        annotation_position="bottom right"
    )
...
for t_idx, ticker in enumerate(selected_tickers):
    # stock data
    stock_pd = ticker_values.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()
    ...
    return_rate_pd = return_rates.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()

    # volume dataframe
    stats_volumes_pd = stats_volumes.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()

    # moving average dataframe
    moving_avg_pd = moving_avg.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()

    # correlation dataframe
    corr_col_a = "ticker_a"
    corr_col_b = "ticker_b"
    correlation_pd = correlation.filter((F.col(corr_col_a) == ticker) | (F.col(corr_col_b) == ticker)).toPandas()

    # ad_line dataframe
    ad_line_pd = ad_line.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()

    #rsi dataframe (14 days)
    rsi_pd = rsi.filter(F.col(ColumnNames.TICKER.value) == ticker).toPandas()

    emote_state = "📈" if return_rate_pd[mapping_return_column_date.get(roi_time_window)].iloc[-1] > 0 else "📉"
    with st.expander("{} - {} {}".format(ticker, dict_companies[ticker], emote_state), expanded=t_idx == 0):
        st.subheader("{} - {} {}".format(ticker, dict_companies[ticker], emote_state))

        with st.container(border=True):
            # tabs for different insights
            tab_rendement, tab_volume, tab_corr, tab_ad, tab_rsi = st.tabs(
                ["📊 Rendement Mobile & Volume", "📊 Volume", "📊 Corrélation", "〰 A / D Line", "📈RSI"]
            )
            ...
            with tab_rendement:
                ...
                st.write("Variance measures the difference between the daily return and the moving average over time.")
                
                diff_col = f"{ColumnNames.DAILY_RETURN.value}_DIFF_{DEFAULT_MVG_AVG_DAYS}D"
                df_stats = moving_avg_pd[diff_col]
                stats_summary = df_stats.describe().round(4)[["min", "max", "mean"]]
                min_stat, max_stat, mean_stat = stats_summary.loc["min"], stats_summary.loc["max"], stats_summary.loc["mean"]
                min_date, max_date = moving_avg_pd.loc[moving_avg_pd[diff_col].idxmin()][ColumnNames.DATE.value], moving_avg_pd.loc[moving_avg_pd[diff_col].idxmax()][ColumnNames.DATE.value]
                st.markdown(f"""
                ...
                st.plotly_chart(fig_ratio_sharpe, key=f"{ticker}_sharpe", use_container_width=True)
            ...
            with tab_volume:
                ...
                st.plotly_chart(fig_volume, key=f"{ticker}_daily_volume", use_container_width=True)

                # Stats and Insights
                df_stats = stock_pd[ColumnNames.VOLUME.value]
                stats_summary = df_stats.describe().round(4)[["min", "max", "mean", "std"]]
                min_stat, max_stat, mean_stat, std_stat = stats_summary.loc["min"], stats_summary.loc["max"], stats_summary.loc["mean"], stats_summary.loc["std"]
                min_date, max_date = moving_avg_pd.loc[moving_avg_pd[diff_col].idxmin()][ColumnNames.DATE.value], moving_avg_pd.loc[moving_avg_pd[diff_col].idxmax()][ColumnNames.DATE.value]
                st.markdown(f"""
                ...
                if roi_time_window == EnumPeriod.DAY:
                    st.markdown("❌ **Volume by Period not available for a daily or a week analysis.**")
                else:
                    # Create volume by period plot
                    fig_volume_period = fig_volume_by_period(stats_volumes_pd, roi_time_window)
                    st.plotly_chart(fig_volume_period, key=f"{ticker}_volume_period", use_container_width=True)
                ...
                fig_ratio_volume = fig_volume_ratio(stats_volumes_pd, roi_time_window)
                st.plotly_chart(fig_ratio_volume, key=f"{ticker}_volume_ratio", use_container_width=True)
            ...
            # Correlation
            with tab_corr:
                st.write(f"Analyze the correlation between {ticker} and other variables.")
                ...
                if len(selected_tickers) == 1:
                    ...
                    ticker_correlation_df = correlation_pd[
                        [ColumnNames.TICKER.value, col_corr_ticker, ColumnNames.CORRELATION.value]
                    ]
                    ticker_correlation_df = ticker_correlation_df.sort_values(ColumnNames.CORRELATION.value, ascending=False).reset_index(drop=True)
                    ticker_correlation_df["Rank"] = ticker_correlation_df.index + 1

                    # Display correlation data
                    st.subheader("Correlation Table")
                    st.dataframe(
                        ticker_correlation_df[["Rank", "correlation ticker", ColumnNames.CORRELATION.value]],
                        hide_index=True,
                    )
            ...
            # AD line
            with tab_ad:
                st.subheader("📈 A/D Line", help="Measures buying and selling pressure by combining price changes and transaction volume.")
                ...
                **Tip**: Combine the A/D Line with price trends for better confirmation of market movements.
                ...
                st.plotly_chart(fig, key=f"{ticker}_ad", use_container_width=True)

            #RSI
            with tab_rsi:
                st.subheader("📈 RSI", help="The RSI measures the intensity of overbought or oversold conditions.")
                ...
                if time_window in [EnumPeriod.WEEK, EnumPeriod.DAY]:
                    ...
                    # Create RSI plot
                    fig = fig_rsi(rsi_pd, column_rsi_name = f"RSI_{DEFAULT_ADX_RS_DAYS}D")

                    # Display chart in Streamlit
                    st.plotly_chart(fig, key=f"{ticker}_rsi", use_container_width=True)